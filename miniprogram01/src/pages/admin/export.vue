<template>
  <view class="page page-with-nav">
    ...
    <text class="helper-text">请先填写筛选条件，再查看导出预览。未填写时将展示全部前端演示数据。</text>
...
import { getExportPreviewRows, simulateExportAction } from '@/utils/mockData'
...
const previewRows = computed(() => getExportPreviewRows(form))

/** 模拟导出动作并记录日志。 */
const handleExport = () => {
  simulateExportAction(previewRows.value.length)
  showSuccessToast(`已生成 ${previewRows.value.length} 条导出演示数据`)
}
